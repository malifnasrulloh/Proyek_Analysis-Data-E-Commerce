...
def getTotalIncome(order_df: pd.DataFrame, order_item_df: pd.DataFrame, deliveredOnly: bool):
    order_df = order_df[order_df.order_status ==
                        'delivered'] if deliveredOnly == True else order_df
...
def getCorrelatProduct(product_df: pd.DataFrame, order_items_df: pd.DataFrame):
    ...
    return df.groupby(by=['product_weight_g']).agg({'price': 'mean'}).sort_values(by=['product_weight_g'], ascending=False).reset_index().head(500).to_dict()
...
def getMostPurchasesCountries(order_df: pd.DataFrame, customer_df: pd.DataFrame):
    df = pd.merge(order_df[order_df.order_status != 'canceled'],
                  customer_df, how='inner', on='customer_id')
...
def getCorrelatBuyerSellerLocation(order_df: pd.DataFrame, order_items_df: pd.DataFrame, customer_df: pd.DataFrame, seller_df: pd.DataFrame):
    df = pd.merge(order_df, customer_df[[
                  'customer_state', 'customer_id']], how='inner', on="customer_id")
    df = pd.merge(
...
with st.sidebar:
    first_date, last_date = st.date_input(label='Plese select date range', value=[
                                          first_date_order, last_date_order], max_value=last_date_order, min_value=first_date_order)
    search = st.text_input("Check Order ID")
    with st.expander("Result: "):
        st.write(orders.loc[search == orders.order_id])

filtered_orders = orders[(orders["order_purchase_timestamp"] >= str(
    first_date)) & (orders["order_purchase_timestamp"] <= str(last_date))]
...
with col[0]:
    ...
    fig, ax = plt.subplots()
    product_frequent = products.groupby(
        by=["product_category_name"]).product_category_name.count().to_dict()
    product_frequent = dict(
        sorted(product_frequent.items(), key=lambda x: x[1], reverse=True)[:7])
    plt.pie(x=product_frequent.values(), labels=product_frequent.keys(), autopct='%1.1f%%', explode=list(
        map(lambda x: 0.2 if x == max(product_frequent.values()) else 0, product_frequent.values())))
...
with st.container():
    ...
    payDistribute = getProductPaymentDistribute(
        products, order_items, order_payments).to_dict()
